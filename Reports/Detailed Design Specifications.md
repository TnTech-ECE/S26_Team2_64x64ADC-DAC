...
## Schematics
As is standard practice for many engineering firms, the team has designed and manufatured an evaluation PCB to be able to test every design concept mentioned in this document, but on a smaller scale. This board was designed in a way which maximizes the developed code, allowing most of it to be re-used on the full production-level PCB. Hence, some elements are redundent (namely, the shift register fanouts). While unnecessary for the function of this circuit, they are included, as a 16 channel version of this board is dependent on the use of shift registers for output expansion.

The evaluation PCB includes a high and low pass filter, a single parametric filter, a single 4-channel ADC, and a single 8-channel DAC. The PCB contains headers for a single balanced audio input, which can be manually assigned via jumpers to ADC channels 1-4. Two headers for two balanced audio outputs are available, which can be assiged to DAC outputs 1-4 and 5-8, respectively. There are three ribbon cables attached to the PCB which handle all of the digital communication and can be plugged into the AC701 FPGA evaluation PCB's FMC expansion card. Finally, there are two SMA connectors meant for direct connection from the AC701's low-jitter clock generation to the evaluation PCB's TDM MCLK inputs. The ADC receives a discrete 24.576 MHz MCLK, and the DAC receives a discrete 24.576 MHz MCLK. 

Each page from the schematic can be found below, with a short description of what is on each page. 
### Root
This is the basic flow and interconnect diagram of the evaluation PCB. Here, the major control signals are shown
<img width="1745" height="1202" alt="image" src="https://github.com/user-attachments/assets/f2f87cb8-ec31-49ac-92ec-5675876017c6" />

### ADC
Contains the ADC chip, the header to assign the input circuitry to channels 1-4 and the precision reference used for both the ADC and DAC
<img width="1745" height="1202" alt="image" src="https://github.com/user-attachments/assets/de8e40d4-b8ee-4a15-8e01-621d1830d008" />

### DAC 
Contains the DAC chip, the header for channel assignment and the output drive circuitry
<img width="1489" height="1226" alt="image" src="https://github.com/user-attachments/assets/1271201c-7f66-4188-921f-8b0ae4e2eaf1" />

### MCU
Contains the MCU, level shift circuit, and signal grouping types
<img width="1760" height="1287" alt="image" src="https://github.com/user-attachments/assets/0ef1be80-6fa6-44a0-a59c-bd3477937f5d" />

### Input Stage
Contains the input stage and relay switching for the analog filters
<img width="2003" height="1070" alt="image" src="https://github.com/user-attachments/assets/7d22a65d-7eae-4449-bbba-ad9fa8bff4a9" />

### High Pass, Low Pass Filters
Contains the high and low pass circuit along with the digipot and capacitor decade circuit
<img width="1560" height="1236" alt="image" src="https://github.com/user-attachments/assets/9ce42935-04b2-46ca-9e1d-792a13cba40f" />

### Parametric Filter
Contains the capacitor decade circuit and the parametric filter
<img width="1225" height="1227" alt="image" src="https://github.com/user-attachments/assets/cbecc7a0-033b-4f9d-97cf-2bc30decb564" />

### Parametric Digipots
Contains the digipots controlling frequency and gain for the parametric filter
<img width="1897" height="1107" alt="image" src="https://github.com/user-attachments/assets/f033e89e-213e-49b2-96be-6e02dcb5587f" />

### Power
Contains all of the voltage regulators, as well as coupling of GND and GNDD
<img width="1750" height="1206" alt="image" src="https://github.com/user-attachments/assets/2a4dc030-1a49-415d-98fd-5221da962c10" />

### Fanout
Contains the fanout shift registers
<img width="1750" height="1206" alt="image" src="https://github.com/user-attachments/assets/bfef0af9-294b-44ac-95c5-696206029e0a" />

## PCB Layout
Below are the four layers of the PCB. A description of each layer accompanies each image.

### Top Layer 1
Analog signal, Ground
<img width="2360" height="1186" alt="image" src="https://github.com/user-attachments/assets/bc01e925-d178-45fa-b827-c2380fe20064" />

### Inner Layer 2
Ground
<img width="2360" height="1186" alt="image" src="https://github.com/user-attachments/assets/8ac25e9a-5b8f-40b8-b36d-24a184c9901c" />

### Inner Layer 3
Power, Digital Ground Return
<img width="2360" height="1186" alt="image" src="https://github.com/user-attachments/assets/dc58655a-828b-494f-bfb5-69e52af5b73d" />

### Bottom Layer 4
Digital Signal
<img width="2360" height="1186" alt="image" src="https://github.com/user-attachments/assets/92595388-687a-43ad-afba-5e2b140225cf" />

## Bill of Materials
Below is the bill of materials used to order parts to build the evaluation PCB: 
<img width="2002" height="1253" alt="image" src="https://github.com/user-attachments/assets/44761466-1b15-4389-a1f8-e4dd0f6f8017" />
...
| 5V DC, +/-18 DC -> Power Supply | Power | Input | LDOs generate 3.3V, 3.3VQ, 4.5VQ,1.2V, 2.5V, +/-15V |
...
| Power | +/-2v5, GNDD | Input | Analog supply for MC33079; digital supply for digipots |
...
| Power | +/-2v5, GNDD | Input | Analog supply for opamps; digital supply for digipots |
...
| Power | +3.3V, +1.2V (core), +5V, +4v5Q, GND | Input | From power subsystem |
...
| Power | +1.2V (core), +3.3V, +3v3Q, +/-15V, GND | Input | Core digital + analog opamp supply from power subsystem |
...
| Level Translation | Digital | Bidirectional | SN74AVC4T245PW: 3.3V (MCU side) <-> 2.5V (analog side) |
| Power | +3.3V, +2.5V, GNDD | Input | 3.3V for MCU; 2.5V for translator analog side |
...
| Power | +2v5, GNDD | Input | Digital supply |
...
| Interface | Signal Type | Direction | Details |
|---|---|---|---|
| 5V DC In | Power | Input | External power supply, barrel jack or screw terminal |
| +/-18V DC In | Power | Input | External power supply, barrel jack or screw terminal |
...
| +3v3Q Out | Power | Output | DAC precision reference|
| +4v5Q Out | Power | Output | ADC precision reference|
| GND / GNDD / GND1 | Power | -- | Separate digital, analog, and clock ground planes |

**Shall Statements:**

1. The power supply shall accept a 5V, +/-18V DC input and generate all required voltage rails.
